...
//定义全局变量   去接受页码对应编码值
var page_cache = [];
var get_textdiv_start_index = 0;
var get_textdiv_end_index = 0;
var get_textdiv = "";
...
/******/ (function(modules) {
  ...
  /******/ [
    ...
    /***/ function(module, exports, __webpack_require__) {
                  ...
                  while (1) {
                    switch ((_context5.prev = _context5.next)) {
                      case 0:
                        ...
                        // let highLightWords = ["华软"];
                        // wordHighLight('广州大学华软软件学院');
      ...
      //     phraseSearch: true, // 支持整段文字匹配,如果时多个词的匹配只能是false
      //     caseSensitive: false, // 默认为false,搜索时忽略大小写
      //     highlightAll: false, // 设为true即关键词全部高亮
      //     // findPrevious: true,
      //   };
      //   PDFViewerApplication.findController.executeCommand("find" + evt.type, {
      //     //搜索执行函数
    ...
    /***/ function(module, exports, __webpack_require__) {
      ...
      var defaultOptions = {
        ...
        defaultUrl: {
          // value: "compressed.tracemonkey-pldi-09.pdf",//默认的值
          value: "",
          kind: OptionKind.VIEWER,
        },
        defaultZoomValue: {
        ...
        _createClass(TextLayerBuilder, [
          ...
          },
          {
            ...
            value: function _bindMouse() {
              ...
              // console.log('2222',div)
              var expandDivsTimer = null;
              div.addEventListener(
                "mousedown",
                function(evt) {
                  //当前页相应鼠标按下操作,清除div选取列表数据
                  get_textdiv = "";
                  if (
                  ...
                  var start_index = document.getElementById(
                    "get_textdiv_start_index"
                  );
                  //记录鼠标开始点击位置
                  ...
                  for (let i = 0; i < div.children.length; i++) {
                    div.children[i].addEventListener(
                      "mousedown",
                      function(env) {
                        // console.log("i1", i);
                        start_index.value = i;
                        get_textdiv_start_index = i;
                      }
                    );
                  }
                  var mousedown_page_num = document.getElementById(
                    "mousedown_page_num"
                  );
                  mousedown_page_num.value=div.parentNode.getAttribute('data-page-number');
                  // console.log('down_page',div.parentNode.getAttribute('data-page-number'));
                  // console.log(
                  //   "get_textdiv_start_index",
                  //   get_textdiv_start_index
                  // );
              ...
              div.addEventListener("mouseup", function() {
                ...
                var end_index = document.getElementById(
                  "get_textdiv_end_index"
                );
                //记录鼠标完成点击的位置
                for (let i = 0; i < div.children.length; i++) {
                  div.children[i].addEventListener("mouseup", function(env) {
                    // console.log('i2',i)
                    end_index.value = i;
                    get_textdiv_end_index = i;
                  });
                }
                // console.log("get_textdiv_end_index", get_textdiv_end_index);
                //判断选中div的数目，获取内容
                if (
                  Math.abs(get_textdiv_end_index - get_textdiv_start_index) <=
                  100
                ) {
                  if (get_textdiv_end_index < get_textdiv_start_index) {
                    // for (
                    //   let j = get_textdiv_end_index;
                    //   j <= get_textdiv_start_index;
                    //   j++
                    // ) {
                    //   get_textdiv += div.children[j].innerHTML;
                    // }
                    let tempNum = get_textdiv_end_index;
                    get_textdiv_end_index = get_textdiv_start_index;
                    get_textdiv_start_index = tempNum;
                  }
                  for (
                    let j = get_textdiv_start_index;
                    j <= get_textdiv_end_index;
                    j++
                  ) {
                    get_textdiv += div.children[j].innerHTML;
                  }
                  ...
                  //   "get_textdiv_start_index"
                  // );
                  // start_index.value = get_textdiv_start_index;
                  // console.log(
                  //   "get_textdiv_start_index",
                  //   get_textdiv_start_index
                  // );
                  ...
                  //   "get_textdiv_end_index"
                  // );
                  // end_index.value = get_textdiv_end_index;
                  // console.log("get_textdiv_end_index", get_textdiv_end_index);
                  var html_input_getdiv = document.getElementById(
                    "get_textdiv"
                  );
                  // console.log("get_textdiv", get_textdiv);
                  html_input_getdiv.value = get_textdiv;
                }
                var mouseup_page_num = document.getElementById(
                  "mouseup_page_num"
                );
                mouseup_page_num.value=div.parentNode.getAttribute('data-page-number');
                // console.log('up_page',div.parentNode.getAttribute('data-page-number'));
              },true);
            },
          },
        ]);
